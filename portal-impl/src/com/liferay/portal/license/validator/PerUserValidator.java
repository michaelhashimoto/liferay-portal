...
public class PerUserValidator extends LicenseValidator {
	...
	public void doValidateVersion3(License license) throws Exception {
		if ((license.getMaxConcurrentUsers() <= 0) &&
			(license.getMaxUsers() <= 0)) {
		...
		if (productId.equals(LicenseManager.PRODUCT_ID_PORTAL)) {
			...
			if ((license.getMaxConcurrentUsers() > 0) &&
				...
				throw new Exception(
					"The property live.users.enabled must be true to use " +
						"this license");
			}
		}
	...
	private static final String[] _VALID_TYPES = {License.TYPE_PER_USER};
