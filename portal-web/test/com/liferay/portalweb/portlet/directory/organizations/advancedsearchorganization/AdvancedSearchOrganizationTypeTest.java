 ...
 */
public class AdvancedSearchOrganizationTypeTest extends BaseTestCase {
	public void testAdvancedSearchOrganizationType() throws Exception {
		int label = 1;
		...
		while (label >= 1) {
			...
			case 2:
				...
				for (int second = 0;; second++) {
					...
					try {
						if (selenium.isVisible("//select[@id='andOperator']")) {
							break;
						}
					}
					catch (Exception e) {
				...
				selenium.select("//select[@id='type']",
