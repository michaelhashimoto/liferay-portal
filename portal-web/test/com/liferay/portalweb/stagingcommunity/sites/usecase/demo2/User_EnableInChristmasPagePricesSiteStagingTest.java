 ...
 */
public class User_EnableInChristmasPagePricesSiteStagingTest
	extends BaseTestCase {
	public void testUser_EnableInChristmasPagePricesSiteStaging()
		throws Exception {
		int label = 1;
		...
		while (label >= 1) {
			...
			case 2:
				...
				assertEquals(RuntimeVariables.replace(
						"The page Prices is not enabled in Christmas, but is available for other pages variations."),
					selenium.getText(
						"//div[@id='_170_layoutRevisionDetails']/div"));
				assertEquals(RuntimeVariables.replace("Enable in Christmas"),
					selenium.getText("//button/span[.='Enable in Christmas']"));
				selenium.clickAt("//button/span[.='Enable in Christmas']",
					RuntimeVariables.replace("Enable in Christmas"));
