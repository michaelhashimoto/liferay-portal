 ...
 */
public class AddCalendarEventFuture4DaysSiteTest extends BaseTestCase {
	public void testAddCalendarEventFuture4DaysSite() throws Exception {
		...
		selenium.type("//td[@id='cke_contents__8_editor']/textarea",
			RuntimeVariables.replace("Calendar Future Day4 Event Description"));
		assertEquals(RuntimeVariables.replace("Source"),
			selenium.getText("//span[.='Source']"));
		selenium.clickAt("//span[.='Source']",
			RuntimeVariables.replace("Source"));
		...
		selenium.waitForText("//body", "Calendar Future Day4 Event Description");
		selenium.selectFrame("relative=top");
		...
		assertEquals(RuntimeVariables.replace(
			...
			selenium.getText(
				"xPath=(//td[contains(@id,'_8_ocerSearchContainer_col-title')])[contains(.,'Calendar Future Day4 Event Title')]"));
	}
}