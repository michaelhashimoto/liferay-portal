...
public class PoshiRunnerValidation {
	...
	protected static void validateCommandElement(
		...
		if (returns == null) {
			...
			for (Element returnElement : returnElements) {
				String returnVariable = returnElement.attributeValue(
					"name");
	...
	protected static void validateExecuteElement(
		...
		if (!childElements.isEmpty()) {
			...
			List<String> possibleChildElementNames =
				Arrays.asList("var", "return");
	...
	protected static void validateMacroReturns(
		...
		List<String> returns = PoshiRunnerContext.getMacroCommandReturns(classCommandName);

		String returnVariable = returnElement.attributeValue("from");

		if (!returns.contains(returnVariable)) {
			_exceptions.add(
				new Exception(returnVariable +
					" not specified as a return variable\n" + filePath + ":"
						+ element.attributeValue("line-number")));
		}
	}
