 ...
 * @author Peter Yoo
 */
package com.liferay.jenkins.results.test;
...
import com.liferay.jenkins.results.parser.test.FailureMessageGeneratorTest;
import com.liferay.jenkins.results.parser.test.ResultsParserTest;
...
public class TestDataGenerator {
	...
	public void createTestData(URL jenkinsReport) throws Exception {
		...
		for (Node node : testNodeList) {
			...
			if (parent.getText().endsWith("FAILURE")) {
				...
				try {
					String testGroupRootPath = downloadTestData(testDefinitionName, url);
					_test.createExpectedResultsFile(project, new File(testGroupRootPath));
					i++;
				}
				catch (FileNotFoundException e) {
					System.err.println("Data was not found on server for " +
							url + " skipping.");
				}
			}
			else {
				System.out.println("Skipping non-failure test: " + parent.getText());
			}
			
		}
		System.out.println("Test case creation is complete. " + i + " test groups were created.");
	}	
	
	protected String downloadTestData(String testName, String url) throws Exception {
		...
		if (testGroupName.endsWith("-")) {
			testGroupName = testGroupName.substring(0, testGroupName.length()-1);
		}		
		...
		try {
			...
			File consoleFile = new File(testGroupLogTextDir.getPath() + "/progressiveText");
	...
	public TestDataGenerator(ResultsParserTest test) {
		_test = test;
	}
	...
	private ResultsParserTest _test;
