...
public class JSUnitTestClassFile extends BaseTestClassFile {
	...
	protected JSUnitTestClassFile(File file) throws IOException {
		super(file);
	...
	/**
	 * Returns the name of a test that an expression names, as a placeholder
	 * that matches whatever the expression evaluates to.
	 */
	...
	/**
	 * Returns the name of the test that the argument at <code>index</code>
	 * declares, or <code>null</code> if the argument is not a string literal.
	 */
	...
	private Title _getValue(String content, int index) {
		...
		// The value of anything but a literal is not known until the test runs
	...
	/**
	 * Returns <code>true</code> if <code>word</code> declares a group of tests,
	 * including the words that skip or focus the group.
	 */
	...
	/**
	 * Returns <code>true</code> if <code>value</code> is a literal, whose value
	 * is known without running the test.
	 */
	...
	/**
	 * Returns <code>true</code> if the slash at <code>index</code> opens a
	 * regular expression rather than divides, which is decided by the token
	 * that precedes it.
	 */
	private boolean _isRegexStart(String content, int index) {
		int length = content.length();

		// A slash that precedes ">" closes a JSX tag
		...
		// A slash that follows "<" closes a JSX tag
	...
	/**
	 * Returns <code>true</code> if <code>word</code> declares a test, including
	 * the words that skip or focus the test.
	 */
	...
	private void _parse(
		...
		while (index < length) {
			...
			if (c == '{') {
				...
				// Only the brace that opens the body of the callback holds
				// the group, so that a parameter that destructures an object
				// does not hold it
			...
			if (title == null) {
				...
				// The name is an expression, so it is not known until the test
				// runs
			...
			if (_isDescribeWord(word)) {
				...
				// The names of the tests of an "each" table are known, unless
				// a placeholder formats the value rather than prints it
				...
				if ((rows == null) || rows.isEmpty() || title.dynamic ||
					...
					_testClassFileMethods.add(
						TestClassFileFactory.newTestClassFileMethod(
							dynamic || describeDynamic || uncertain,
	...
	/**
	 * Returns the index after the regular expression that starts at
	 * <code>index</code>.
	 */
	...
	private final List<TestClassFileMethod> _testClassFileMethods =
		new ArrayList<>();
